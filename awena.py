...
def _external_request(query, lang):
	headers = {
	...
	return requests.get("https://www.wikidata.org/w/api.php", headers=headers, params=params, timeout=5)
...
class Crawler:
	...
	##### CONSTRUCTOR #####
	def __init__(self,	lang='en', connector=None):
	...
	def get_subclass(self, id, rel):
		if rel is None:
			return []
		query = """
		select ?p ?o where {
		<"""+"http://www.wikidata.org/entity/"+str(id)+"""> <"""+rel+"""> ?o .
		}
		"""
		return self.connector.query(query)

	def inverse_subclass(self, id, rel):
		if rel is None:
			return []
		query = """
		select ?s where {
		?s <"""+rel+"""> <"""+"http://www.wikidata.org/entity/"+str(id)+""">.
		}
		"""
		return self.connector.query(query)
	...
	def search(self, query, subclass=None, obj=None, find_object=True):
		return self._request(query,False, subclass, obj, find_object)
	...
	def _request(self,query=False,id=False,subclass=None, obj=None, find_object=True):
		headers	= {
			"User-Agent"	: Crawler.USER_AGENT
		}
		if id:
		...
		data = _external_request(query,self.lang)
		result= data.json()
		self.n+= 1
		if 'error' in result:	
